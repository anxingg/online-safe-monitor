...
public class NotifyViewDao extends BaseDao<NotifyView,Integer>{
	 ...
	 * @return int
	 */
	public int getUnReadNotifyCount(int userId){
		String hql = "select count(*) from Notify where status = 2 and (sysdate <= endDate or endDate is null) and columnId=1 and id not in (select notify.id from NotifyView where createUser.userId="+userId+")";
		List<Object> list = super.find(hql);
		int unreadCount = 0;
		for(int i=0; i<list.size(); i++){
			unreadCount = Integer.parseInt(list.get(i).toString());
		}
