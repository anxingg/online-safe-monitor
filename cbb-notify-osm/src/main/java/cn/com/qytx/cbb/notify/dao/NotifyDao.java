...
public class NotifyDao extends BaseDao<Notify,Integer>{
	...
	public Page<Notify> viewList(Pageable pageable, int notifyType,String searchWord,Integer columnId,Integer userId,Integer isShowOut) {
		...
		StringBuffer sqlCondition = new StringBuffer(" status = 2 and columnId="+columnId);
		if(notifyType!=0){
		...
		if(isShowOut!=null&&isShowOut==1){
			sqlCondition.append(" and (sysdate >= beginDate or beginDate is null)");
		}
		sqlCondition.append(" and (sysdate <= endDate or endDate is null)");
...
//			value.add(calendar.getTime());
	...
	public Notify saveOrUpdate(Notify entity) {
		entity.setIsForkGroup(TransportUser.get().getIsForkGroup());
	...
	public Page<NotifyStatisticsResult> findStatisticsResultByPage(
			Pageable pageable, NotifyStatisticsResult nsr) {

		/*
select notify_type, count(*) as ct, columnid
from tb_cbb_notify
where is_delete = 0 
    --and columnid = 2 --2表示政策法规
    --and create_date >= sysdate --发布时间开始
    --and create_date <= sysdate --发布时间结束
    --and notify_type = 1 
group by notify_type, columnid
ORDER by notify_type ASC
;
		 */
		Integer columnId = nsr.getColumnId();
		Date beginDate = nsr.getBeginDate();
		Date endDate = nsr.getEndDate();
		Integer notifyType = nsr.getNotifyType();
		
		List<Object> params = new ArrayList<Object>();
		String contentStatement = "SELECT notify_type, count(*) as ct ";
		String countStatement = "SELECT count(*) from ( SELECT count(id) as ct ";
		StringBuilder sbr = new StringBuilder();
		sbr.append("from tb_cbb_notify ").append(
				"where is_delete = 0 ");
		
		//栏目ID
		if (columnId != null) {
			sbr.append("and columnid = ? ");
			params.add(columnId);
		}

		//创建开始时间
		if (beginDate != null) {
			sbr.append("and create_date >= ? ");
		...
		if (notifyType != null && notifyType != 0) {
			sbr.append("and notify_type = ? ");
			params.add(notifyType);
		}

		sbr.append("group BY notify_type, columnid ");
		...
		sbr.append("order by notify_type ");
...
//		System.out.printf("pageable.getPageSize()=%d\n",pageable.getPageSize());
		...
		List<NotifyStatisticsResult> content = new ArrayList<NotifyStatisticsResult>();
		if (list != null && !list.isEmpty()) {
			for (Object[] objs : list) {
				//notify_type, count(*) as ct
				content.add(new NotifyStatisticsResult(
	 ...
	 * @return
	 */
	public List<Notify> getTopFiveNotifysByColumnId(Integer columnId) {
		String hpql = " columnId=?1 and status=2";
		hpql += " and (sysdate >= beginDate or beginDate is null)";
		hpql += " and (sysdate <= endDate or endDate is null)";
		List<Notify> list = super.unDeleted().findAll(hpql, new Sort(new Sort.Order(Sort.Direction.DESC, "createDate")),columnId);
		List<Notify> newList = new ArrayList<Notify>();
		if (list!=null&&list.size()>0) {
			if (list.size()>=5) {
				for (int i = 0; i < 5; i++) {
