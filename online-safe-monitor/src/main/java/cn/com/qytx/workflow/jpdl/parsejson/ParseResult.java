package cn.com.qytx.workflow.jpdl.parsejson;
...
//	private static int NODE_ORDER = 1 ;
	...
	public List<NodeObject> getJpdlNode(){
		...
		for(Iterator it = sets.iterator(); it.hasNext();){
			...
			//如果是判断节点，则设置EL表达式
			if(no.getType().equals(WorkflowConstans.NODE_TYPE_DECISON)
		...
		for(int i=0;i<result.size();i++ ){
			...
			if(nodeObject.getType().equals(WorkflowConstans.NODE_TYPE_START)){
