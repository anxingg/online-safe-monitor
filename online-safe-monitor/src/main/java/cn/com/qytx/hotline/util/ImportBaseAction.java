...
public class ImportBaseAction extends BaseActionSupport
{
     ...
     * @return
     */
    protected boolean initErrorFile()
    {
        if (errorFileName == null || "".equals(errorFileName))
        {
            ...
            if (!filePath.isDirectory())
            {
                ...
                {
                	logger.error("initErrorFile error",e);
                    return false;
                }
            }
    ...
    protected String getImportResult()
    {
        ...
        if (OldRows > 1)
        {
            ...
            {
            	if(skipNum>0){
            		result = "<span class=' txt_orange ml20'>追加完毕！追加成功" + successNum + "条记录，跳过"+skipNum+"条记录，失败" + errorImportNum
            				+ "条记录</span>&nbsp;<a href='" + basePath + "/upload/" + errorFileName
            				+ "' class='ml10' >下载失败记录</a>";
            	}else{
            		result = "<span class=' txt_orange ml20'>追加完毕！追加成功" + successNum + "条记录，失败" + errorImportNum
            				+ "条记录</span>&nbsp;<a href='" + basePath + "/upload/" + errorFileName
            				+ "' class='ml10' >下载失败记录</a>";
            	}
            }
