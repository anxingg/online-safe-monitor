package cn.com.qytx.hotline.knowledge.action;
...
import cn.com.qytx.hotline.knowledge.domain.KnowledgeType;
import cn.com.qytx.hotline.knowledge.service.IKnowledgeType;
...
{
    private final static MonitorLogger loger =new Log4jImpl(KnowledgeActionType.class);
    ...
    public String selectTreeKnowledge()
    {
        ...
        if (isOnly == null)
        {
            isOnly = Integer.valueOf(PropertiesUtil.get("isOnly", "0"));
        }
        List<TreeNode> allNodes = knowledgeTypeService.findAllKnowledgeType(containAll, user, isPrivate, isOnly);
        ...
        if (user != null)
        {
            if (vid != null && vid > 0)
            {
                ...
                if (ktype == null || vid.equals(ktype.getVid()))
                {
                    ...
                    else if (knowledgeType.getParentId() == 0 && knowledgeType.getIsPrivate() == 0)
                    {
                        a = 3;// 顶级不能修改
                    }
                    else
    ...
    public String deleteType()
    {
        ...
        if (user != null)
        {
            ...
            int a = 0;
            if (ktype.getIsForkGroup().intValue() != user.getIsForkGroup().intValue())
            {
                a = 3;
            }
            else if (ktype.getParentId() == 0 && ktype.getIsPrivate() == 0)
            {
                a = 4;
            }
            else
            {
                a = knowledgeTypeService.deleteById(vid);
            }
            ajax(a);
        }
        return null;
