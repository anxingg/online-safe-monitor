...
$(document).ready(function() {
	...
	// 定期访问事务提醒信息
	getAffairs();
    newAffairsTitleTime = setInterval("getAffairs()", 60000);

	// 定期访问微讯信息  及时消息
   getNewMessage();
  getNewMessageTime = setInterval("getNewMessage()", 60000);
	
	// 快速请求事务提醒 要求每隔3秒钟发送一次请求获取是否有最新的未接受的事务提醒记录信息
	quickAffairsTimer = setInterval("getQuickAffair()", 30000);
...
function exit() {
	art.dialog.confirm('确认注销吗？    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ', function() {
...
function getQuickAffair() {
	$.ajax({
	    ...
	    success : function(data) {
		    if (data == "success") {
			    // 变更提示信息,修改为右下角提示
			    $("#affairsEm").removeClass("icon_work");
			    $("#affairsEm").addClass("newaffairs");
		    }else{
		    	$("#affairsEm").removeClass("newaffairs");
			    $("#affairsEm").addClass("icon_work");
		    }
	    }
	});
}
			            ...
			            for ( var i = 0; i < data.length; i++) {
				            if (i == 0) {
					            ...
					            getUserMessage(data[i].userId);
						             ...
						             sb.Append(data[i].userName+data[i].sendTimeStr);
						             //sb.Append("<span>15:30</span></li>"); 
				               }
			            }
 ...
 */
function send_msg() {
	art.dialog.open(basePath + "logined/message/send_message.jsp?type=box", {
	    id : "send_message",
	    title : "发送即时消息",
	    width : 800,
	    height : 450,
...
function openSendMessage(userId, userName, contentInfo) {
	var url = basePath + "logined/message/alert_send_message.jsp?type=box&userInfoId=" + userId + "&userInfoName="
