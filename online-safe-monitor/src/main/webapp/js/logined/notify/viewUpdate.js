				...
				success : function(data) {
					if(data){
						...
						if( $("#columnId").val() == 1 ){
							...
							//政策法规
							$("#banbuGroup").val(data.banbuGroup);
							var banbuDate = data.banbuDate;
							if(banbuDate != null){
								banbuDate=data.banbuDate.replace("年","-").replace("月","-");
								$("#banbuDate").val(banbuDate.substring(0,10));
							}
							var beginDate = data.beginDate;
							if(beginDate != null){
								beginDate=data.beginDate.replace("年","-").replace("月","-");
								$("#beginDate").val(beginDate.substring(0,10));
							}
							//$("#banbuDate").val(data.banbuDate);
