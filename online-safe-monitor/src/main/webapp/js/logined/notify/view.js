...
//		},
		success : function(data) {
			...
			if(data.approveDate == null){
				$("#createTimeStr").text("发布时间：" + data.differenceTime);
			}else{
				$("#createTimeStr").text("发布时间：" + data.differenceTime);
			}			
			$("#createUserName").text("发布人："+data.ids);
...
function signCheck(notifyId) {
	var notifyId = $("#notifyId").val();
	if ($("#returnType").val() != 1 && $("#returnType").val() != 11){
		return;
	}else{
		dataParam = {
				'notifyId' : notifyId
...
function goback() {
	...
	}else if(returnType==11){//弹出公告
		window.location.href = basePath + 'wh/logined/viewList.jsp?id=' + columnId;
	}
}
