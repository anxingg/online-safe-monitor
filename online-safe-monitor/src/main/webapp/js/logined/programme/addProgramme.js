...
$(document).ready(function() {
	...
	if (isUpdate) {
		...
		//deal with the remind type checkbox
		if (remaindTypeG == 0) {
		} else {
			$("#remaidSpan").show();
		...
		if (hasEndDay == '1') {
			...
			if (isAllDay == 0) {
				...
				$("#setEndMinuInput").val(endMinu);
			}


			$("#isSysRemaind").attr("checked", "checked");
			// deal with repeat type
		...
		$("#repeatType").find("option")[parseInt(repeatTypeG)].selected = true;
	} else {
...
function dealRepeatType(){
		...
		$("#repeatType").empty();
		$("#repeatType").append('<option value="0">不重复</option>'
...
//		alert(begTime)
		...
		if(endTime==''||endTime==undefined||begTime==''||begTime==undefined){
...
//			alert(begTimeArray)
			var beg = Date.parse(begTimeArray[0]+"/"+begTimeArray[1]+"/"+begTimeArray[2]);
			var end = Date.parse(endTimeArray[0]+"/"+endTimeArray[1]+"/"+endTimeArray[2]);
