...
$("#page").attr("src",basePath+"/workflow/process!editProcess.action?processAttributeId="+$("#processAttributeId").val());
  ...
  $('#searchProcess').autocomplete(datas, { 
                    ...
                    formatItem: function(row, i, max) {
                         return i + '/' + max + ':' +row.cateType+' '+ row.name ;//做格式处理
                    },
                    formatMatch: function(row, i, max) {
                         return row.name+row.cateType;//做匹配的数据格式
                    },
