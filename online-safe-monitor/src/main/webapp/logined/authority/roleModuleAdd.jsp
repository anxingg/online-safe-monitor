...
<div class="anthority">
		<div class="title_an">
			<span class="name">功能权限维护</span>
				<c:if test="${requestScope.roleInfo.roleType!=0 || (requestScope.roleInfo.roleType==0 && requestScope.roleInfo.roleCode!='superadmin')}">
					<input id="addRoleModule"
						name="addRoleModule" type="button" value="保  存" class="formButton_green" />
				</c:if>
		<input id="cancelRoleModule"
				name="cancelRoleModule" type="button" class="formButton_grey" 
				onclick="javascript: goback();" value="返  回" />
		</div>
