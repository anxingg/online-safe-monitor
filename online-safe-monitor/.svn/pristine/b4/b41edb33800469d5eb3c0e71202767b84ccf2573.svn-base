...
@Repository
public class DangerChemicalsDao extends BaseDao<DangerChemicalsInfo, Serializable>
	...
	public Page<DangerChemicalsInfo> findByPage(Pageable pageable,
		...
		//部门ID
		Integer groupId = dangerChemicals.getGroupId();
	...
	public boolean usedThisName(Integer id, String name, int which) {
		...
		StringBuffer sbf = new StringBuffer(" isDelete = 0");
		sbf.append(" and ").append(which == 1 ? "materialName" : "molecularFormula").append(" = ?");
		params.add(name);
		if(id != null){
			sbf.append(" and vid != ?");
			params.add(id);
		...
		return ((which != 1 && which != 2) || StringUtils.isBlank(name)) ? false : haveSome(getCollectionSize(super
				.findAll(sbf.toString(), params.toArray())));
	}
