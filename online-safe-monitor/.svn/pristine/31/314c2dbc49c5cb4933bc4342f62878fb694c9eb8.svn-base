				...
				success : function(data) {
					...
					for (var i = 0; i < jsonData.length; i++) {
						if(sid!=""&&sid==jsonData[i].id){
							...
							// 获取第一个维表数据
							getTypeList();
							html += "<p class='menu-p on' onclick='set(\""
									+ jsonData[i].infoType + "\",\""
									+ jsonData[i].id + "\",\""
									+ jsonData[i].value + "\",this);'  > <i class='menu-i on'  id='"
									+ jsonData[i].infoType + "'></i><a href='javascript:void()' >"
									+ jsonData[i].name + "</a></p>";
...
//	var infoType = $("#infoType").val();
	var infoType = _infoType;
	var id = _sysTypeId.substring(4);
	var paramData = {
		'infoType' : infoType,
		'sysTag' : sysTag,
		'grade' : _grade,
...
//				});
	...
	art.dialog.open(url, {
	    ...
	    ok : function(){
	    	...
	    	var parentId = _sysTypeId.substring(4);
	    	if(iframe.addType(typeValue,sysType,parentId,_grade)){
	    		refreshTree();
	    	};	    	
	    	return false;
	    },
...
function updateTypeOpen(vid,vname,typeValue,vorder) {
	...
	art.dialog.open(url, {
	    ...
	    ok : function(){
	    	var iframe = this.iframe.contentWindow;
	    	var parentId = _sysTypeId.substring(4);
	    	if(iframe.updateType(id,1,typeValue,parentId)){
	    		refreshTree();
	    	};	    	
	    	return false;
	    },
		...
		success : function(data) {
			var param = {
					...
					click:	function(nodes){
						zTreeOnCheckResult(nodes);
					},
					loadComplete:function(){
						var zTreeObject = $.fn.zTree.getZTreeObj("groupUserTree");
						var nodeArr = zTreeObject.getNodes();
						var firstId = nodeArr[0].id;
						var node = zTreeObject.getNodeByParam("id", firstId, null);
...
function zTreeOnCheckResult(nodes){
	if(nodes.length>0){
		_sysTypeId=nodes[0].id;
		_infoType=nodes[0].obj.infoType;
		_value=nodes[0].obj.value;
		_grade=nodes[0].obj.grade;
		$("#infoType").val(_infoType);
		getTypeList();
	}
}