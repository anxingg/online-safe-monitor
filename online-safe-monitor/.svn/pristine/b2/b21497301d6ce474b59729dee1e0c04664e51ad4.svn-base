	...
	$('#myTable').dataTable({
		...
		"aoColumnDefs" :[{
			...
			"fnRender" : function(oObj) {
				...
				if(isTop==1){
					return '<a title="'+subject+'" style="text-decoration:none;color:#04578D;text-align:left;'+fontWeight+'"  href="'+ basePath+ 'logined/column/view.jsp?notifyId='+ oObj.aData.notifyId + '&columnId='+columnId+'&returnType=1"><b><font color="red">'+"[置顶]&nbsp;&nbsp;</font>"+subject + '</b></a>';
				}else{
	...
	$.ajax({
		...
		success : function(data) {
			...
			$(list).each(function(i,groupInfo){
				...
				$("#groutName").append(" <p title="+str+" onclick=\"refresh("+groupInfo.groupId+")\" class=\"menu-p\"><i class=\"menu-i\"></i><a href=\"javascript:void()\" >"+name+"</a></p>");
			});
