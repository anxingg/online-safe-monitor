			...
			jQuery("#"+that.conf.id).dataTable({
				...
				'fnServerParams' : function(aoData) {
					...
					for(var key in queryParam){
						aoData.push({"name":key,"value":queryParam[key]});
					}
				},
				...
				for(var i=0;i<_nodes.length;i++){
					...
					if (!_nodes[i].isParent){                    	
						...
						while(_tempNode.getParentNode() != null){
							_treeObj.expandNode(_tempNode.getParentNode(),true);
    ...
    _app.fileupload = function(params){
    	...
    	this.layout = function(){
    		...
    		jQuery("#"+that.params.id).uploadify({
    			...
    			"uploader" : basePath +  "filemanager/uploadfile.action?_clientType=wap&module="+that.params.moduleName||"",
    			...
    			"onUploadProgress" : function(file, bytesUploaded, bytesTotal,totalBytesUploaded, totalBytesTotal) {
    				that.onUploadProgressHelp(file, bytesUploaded, bytesTotal);
    			},
