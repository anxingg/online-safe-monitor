 ...
 * @returns {Boolean}
 */
function forcedRelated(){
	/* 当首页标签页（div_shouye）存在时，强制刷新这个页面。 */
	if (undefined != window.frames["div_messageMangentTab"]) {
		/* 刷新待处理列表 */
		window.frames["div_messageMangentTab"].getSeatMsgTable();//
		return false;
	}
}
