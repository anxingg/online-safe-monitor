...
public class GroupExtDao extends BaseDao<UserInfo, Integer>{
	...
	public void moveOutUserFromAllGroup(String ids){
		String jpql = "delete GroupUser x where userId in ("+ids+")";
		entityManager.createQuery(jpql).executeUpdate();
	}
	...
	public Page<UserInfo> getPageUserListByGroupId(Integer companyId,Pageable page,int groupId,String searchName,Integer sex){
		...
		if(groupId!=0){
			...
			condation += " and userId in (select gu.userId from GroupUser gu where gu.groupId in (select f.groupId from GroupInfo f where isDelete = 0 and groupType = 6))";
		}
