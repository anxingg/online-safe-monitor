...
public class UserAddAction extends BaseActionSupport{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2067722808517056032L;
...
//	        user.setLoginName(user.getPhone());
    		...
    		log.setRefId(user.getUserId());
    		log.setRemark("人员新增成功");
    		log.setUserId(getLoginUser().getUserId());
    		log.setUserName(getLoginUser().getUserName());
    		log.setType(0);//手动添加系统日志
    		logService.saveOrUpdate(log);
	        
	        //add by 吴洲,添加默认角色，MANAGE_ADMIN
	    	//boolean flag = roleService.isExistRoleCode(RoleInfo.MANAGE_ADMIN);
	    	boolean flag = StringUtils.isBlank(roleIds) ? false : roleService.isExistRoleCode(roleIds);
	        if(flag){
	        	RoleInfo role = roleService.findByRoleCode(roleIds);
	        ...
	        if(StringUtils.isNotEmpty(otherGroupJson)){
	        	...
	        	if(otherGroupUsers != null && !otherGroupUsers.isEmpty()){
