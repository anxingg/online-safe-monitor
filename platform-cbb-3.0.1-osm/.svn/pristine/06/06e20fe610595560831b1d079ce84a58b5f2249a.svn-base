	...
	public void getUserByGroupExtId(){
		try{
	        ...
	        if (userList != null) {
	        	...
	        	//获得部门id集合
	        	String groupIds = getGroupIds(userList);
	        	Map<Integer, String> groupMap = groupService.getGroupMapByIds(companyId, groupIds);
...
//	                map.put("role", roleString.toString());
	                ...
	                // 部门path
	                String groupName = "";
	                if (groupMap!=null&&groupMap.get(user.getGroupId())!=null) {
	                	groupName = groupMap.get(user.getGroupId());
					}
	 ...
	 public void moveOutUserFromGroup(){
		 try{
			...
			groupExtService.moveOutUserFromAllGroup(ids);
			userService.deleteUserByIds(getLoginUser().getCompanyId(), ids);;
			 ajax(1);
		 }catch(Exception e){
			 e.printStackTrace();
			 ajax(0);
	 ...
	 * @return
	 */
	private static String getGroupIds(List<UserInfo> list){
		String groupIds = "";
		if (list!=null&&list.size()>0) {
			for (UserInfo info : list) {
				groupIds += ","+info.getGroupId();
			}
		}
		return groupIds;
	}
